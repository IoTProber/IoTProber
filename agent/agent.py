...
        """
        一次性从 evaluation/predict/result/{device_name}_type_prediction.json 加载已有决策结果的IP集合
        Batch-load IPs that already have decision results from the prediction JSON file
        """
        result_path = os.path.join(self.base_path, "evaluation", "predict", "result")
...
        """
        将新决策结果与已有结果合并保存到 evaluation/predict/result/
        Merge new decision results with existing ones and save
        """
        result_path = os.path.join(self.base_path, "evaluation", "predict", "result")
...
        ]:
            filepath = os.path.join(result_path, f"{device_name}_{suffix}.json")
            existing = []
            if os.path.exists(filepath):
                try:
                    with open(filepath, "r", encoding="utf-8") as f:
...
        """
        决策流程: 基于检索结果, 使用Gemini+Claude双LLM联合投票完成设备分类
        Decision pipeline: classify devices using Gemini+Claude joint voting
        based on retrieval results stored in query_db/
...
        print(f"\n{'='*60}")
        print("=== 开始决策流程 (Decision) ===")
        print(f"{'='*60}")
        logging.info("开始决策流程 (Decision)")
...
                result = decision_agent.classify(ip, device_name)
